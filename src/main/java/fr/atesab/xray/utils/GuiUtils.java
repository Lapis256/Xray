...
public class GuiUtils {
    ...
    public static void drawScaledCustomSizeModalRect(int x, int y, float u, float v, int uWidth, int vHeight, int width,
        ...
        bufferbuilder.vertex((float) x, (float) y, 0.0F).texture((float) (u * scaleX), (float) (v * scaleY))
                .color(red, green, blue, alpha);
        try { BufferRenderer.drawWithGlobalProgram(bufferbuilder.end()); } catch(IllegalStateException state) { System.out.println(state.getMessage());}
    }
}
